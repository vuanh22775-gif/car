const express = require('express');
const router = express.Router();
const path = require('path');
const fs = require('fs');
const { protect } = require('../middleware/auth');
const upload = require('../middleware/uploadMiddleware');
const cloudinary = require('../config/cloudinary');

// @desc    Upload ảnh xe (1 hoặc nhiều file)
// @route   POST /api/upload/vehicles
// @access  Private
router.post(
  '/vehicles',
  protect,
  upload.array('images', 10), // tối đa 10 ảnh mỗi lần
  (req, res) => {
    if (!req.files || req.files.length === 0) {
      return res.status(400).json({ success: false, message: 'Không có file nào được upload' });
    }

    let urls;
    if (upload.usingCloudinary) {
      // multer-storage-cloudinary trả URL đầy đủ trong file.path (và public id trong file.filename)
      urls = req.files.map(file => file.path);
    } else {
      // Fallback local dev: trỏ vào static endpoint /uploads
      urls = req.files.map(
        file => `${req.protocol}://${req.get('host')}/uploads/vehicles/${file.filename}`
      );
    }

    res.status(200).json({ success: true, urls });
  }
);

// @desc    Xóa ảnh xe
// @route   DELETE /api/upload/vehicles
// @access  Private
router.delete('/vehicles', protect, async (req, res) => {
  const { url } = req.body;
  if (!url) return res.status(400).json({ success: false, message: 'Thiếu URL' });

  if (upload.usingCloudinary) {
    try {
      // URL dạng: https://res.cloudinary.com/<cloud>/image/upload/v.../moto/vehicles/<public_id>.ext
      const match = url.match(/\/upload\/(?:v\d+\/)?(.+)\.[a-zA-Z0-9]+$/);
      const publicId = match ? match[1] : null;
      if (!publicId) {
        return res.status(400).json({ success: false, message: 'URL không hợp lệ' });
      }
      await cloudinary.uploader.destroy(publicId);
      return res.status(200).json({ success: true, message: 'Đã xóa ảnh' });
    } catch (err) {
      return res.status(500).json({ success: false, message: 'Không thể xóa ảnh trên Cloudinary' });
    }
  }

  // Fallback local dev
  const filename = url.split('/uploads/vehicles/').pop();
  if (!filename) return res.status(400).json({ success: false, message: 'URL không hợp lệ' });

  const filePath = path.join(__dirname, '../../uploads/vehicles', filename);

  fs.unlink(filePath, (err) => {
    if (err && err.code !== 'ENOENT') {
      return res.status(500).json({ success: false, message: 'Không thể xóa file' });
    }
    res.status(200).json({ success: true, message: 'Đã xóa ảnh' });
  });
});

module.exports = router;
